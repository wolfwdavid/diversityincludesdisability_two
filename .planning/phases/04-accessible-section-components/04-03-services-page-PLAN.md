...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/routes/services/+page.svelte"
      provides: "Services route: single h1 + <svelte:head> from seo.services + ServicesDetail"
      contains: "<h1"
  key_links:
...
<objective>
Build the Services detail page (SECT-02): a new `/services` route that renders all four service pillars from
`$lib/content`, each as a `<section aria-labelledby>` with an `<h2>` heading and its summary. The route owns a
single `<h1>` and reads `seo.services` into `<svelte:head>`. Content-driven (RESEARCH Pattern 4) so the four
pillars are structurally sourced from the barrel — no duplicated copy.

Purpose: SECT-02 (four-pillar services detail) with an ordered heading tree (A11Y-02) and prerendered SEO.

Output: `src/lib/components/sections/ServicesDetail.svelte` (+ spec); new `src/routes/services/+page.svelte`.
</objective>
...
<task type="auto">
  <name>Task 2: Create the /services route (+page.svelte)</name>
  <files>src/routes/services/+page.svelte</files>
  <read_first>
    - src/routes/+page.svelte OR src/routes/services (new dir) — none exists yet; create src/routes/services/+page.svelte
  ...
  <acceptance_criteria>
    - File `src/routes/services/+page.svelte` exists; `grep -c '<h1' src/routes/services/+page.svelte` equals 1.
  ...
  <done>/services route renders a single h1 + ServicesDetail + seo.services head; check + spec GREEN.</done>
</task>
...
<success_criteria>
- `/services/` renders all four pillars (barrel-sourced), each an h2 region, under one h1, with prerendered SEO.
</success_criteria>
